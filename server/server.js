...
app.get('/sitemap.xml', async (req, res) => {
  try {
    ...
    console.log('Sitemap generated: ' + products.length + ' products, ' + categories.length + ' categories');
    
    res.send(sitemap);

  } catch (error) {
    console.error('Error generating sitemap:', error);
    res.status(500).header('Content-Type', 'text/plain').send('Error generating sitemap: ' + error.message);
  }
});
